...
wls_model <- lm(Price ~ Mileage, data = data_2, weights = weights)

# Summary of the WLS model
summary(wls_model)
```
The WLS model fits the data well, with significant coefficients and a high R-squared value.
...
We can use the leaps library to access the leaps() and regsubsets() functions, which we will need to apply an exaustive model selection search.
