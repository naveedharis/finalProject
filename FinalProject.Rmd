...
To check for outlier's in y we can use the residuals vs fitted value plot, but since we already used this plot above and commented slightly on the possibility of outliers using the Bonferroni correction to establish the points themselves is a better way.

```{r, echo = FALSE}
### NOTE: JACK WAS NOT ABLE TO GET THIS TO WORK SUCCESSFULLY CAN SOMEONE ELSE TRY?
```
...
new_indices \<- 1:nrow(data) rownames(data) \<- new_indices
...
We see here that our adjusted R-squared increased by about 0.022. Let's check Cook's Distance

cook_full_2 \<- cooks.distance(full_model_2) plot(cook_full_2, ylab="Cook's Distance", main="Cook's Distance", type="l")

```{r, echo = FALSE}
cook_full_2[which.max(cook_full_2)]
```

Our new largest Cook's Distance is 0.11, which is less than our threshold of 0.5. This tells us that our new dataset does not contain significantly influential points.
...
```{r, echo = FALSE}
jackknife_residuals <- rstudent(full_model_2)
jackknife_residuals[which.max(abs(jackknife_residuals))]
qt(.05/(2247*2), 2239)
```

Our largest jackknife residual of 8.99 is larger than our critical value of 4.25, so we reject the null hypothesis that none of our responses are outlying, and conclude that at least one of the used car sale prices is an outlier in the dataset.
...
The following can be determined from looking at the Residuals vs Leverage plot of the model. Despite having relatively large residuals, indices 718, 706, and 1531 are still within the bands of D=0.5 and D=1. Therefore, they are being kept in our analysis.
...
Looking at this summary, we can see that our adjusted R-squared value increased significantly from .72 to nearly .79. The significance of our predictors remained largely unchanged; however, the magnitudes of the betas are drastically different.
...
Looking at the QQ-Residuals plot, the vast majority of the residuals fall on the qq-line. However, a few in partifular stray far from this line - including observations 718 and 706 as previously noted.
