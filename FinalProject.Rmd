...
-------------------------------------------
The following was done by Andy after Jack had pushed the same tasks, so we can delete redunancies later.

## Normality Assumptions
### qq-plots
I am creating a new data set that doesn't include the body type predictor so it is easier to do normality tests 

data_without_Body_Type = subset(data, select = -Body_Type)

```{r, echo = FALSE}
qqnorm(data_without_Body_Type$Price)
qqline(data_without_Body_Type$Price)
```
This plot suggests that our data may be right-skewed, as it is concave up and parabolic in shape. 

```{r, echo = FALSE}
qqnorm(Rstudent_residuals, ylab="R-Student Residuals")
qqline(Rstudent_residuals)
```
...
```
This extremely low p-value suggests we reject the null hypothesis of normality

### Large leverage points
plot(lm.influence(full_model)$hat, ylab="Leverage")
abline(h=2*(8/2919))
lm.influence(full_model)$hat[lm.influence(full_model)$hat>2*(8/2919)]

The car corresponding to index 1036 has the largest leverage point of 0.337. Upon 
inspecting the dataframe this car has over 1.1 million miles, so it makes sense why 
it has such great influence. There are many other notable points looking at this plot
which have leverages over the threshold of 2*(k+1/n), so we should consider eliminating those in the future. 
...
```{r, echo = FALSE}
### NOTE: JACK WAS NOT ABLE TO GET THIS TO WORK SUCCESSFULLY
```
>>>>>>> 48d90f08a3618e4e48257ab78a521460e4d680e9
